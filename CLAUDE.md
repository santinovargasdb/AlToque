...
Marketplace web/PWA que conecta usuarios con profesionales de oficios verificados (plomería, cerrajería, carpintería, etc.) para urgencias y trabajos agendados. Matching por geolocalización. Modelo de ingresos: **suscripción mensual** (spec `docs/superpowers/specs/2026-07-31-modelo-suscripcion.md`); la plataforma NO participa del pago del servicio — cliente y profesional acuerdan precio y medio de pago entre ellos (`jobs.payment_method` y `jobs.final_price` son datos informativos).
...
> - **`pnpm db:push` está PROHIBIDO contra prod**: borraría los objetos que viven fuera de `schema.ts` (PostGIS, policies RLS, `find_nearby_providers`). Solo `db:migrate`.
...
- `src/app/api/` — Webhooks (Mercado Pago), push, cron. El resto de mutaciones son Server Actions.
...
- **Pago del servicio:** entre las partes, fuera de la plataforma. `payment_method` y `final_price` se registran como datos del acuerdo. MP se reserva para el cobro de suscripciones (spec 2026-07-31).
...
| `MP_ACCESS_TOKEN` / `MP_WEBHOOK_SECRET` | Mercado Pago (cobro de suscripciones) |
| `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` / `GOOGLE_MAPS_SERVER_KEY` | Google Maps |
| `RESEND_API_KEY` | Email |
| `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` | Web Push |
| `CRON_SECRET` / `NEXT_PUBLIC_APP_URL` | Cron, URL |

## Reglas No Negociables

1. TypeScript strict. Prohibido `any`; usar tipos inferidos de Drizzle/Zod.
2. Verificar la firma de TODOS los webhooks de Mercado Pago + idempotencia (para suscripciones: índice único sobre `mp_preapproval_id`, spec 2026-07-31).
3. RLS habilitado en todas las tablas.
4. Nunca commitear `.env*`. Validar env vars con Zod al iniciar.
5. Solo profesionales `approved` aparecen en búsquedas/dispatch.
6. Mobile-first: cada vista funciona en 360px de ancho.

## Estado del scaffold

**Steps 1–8 + 10 + 11 + 12 completos** (typecheck/lint/test/build OK). Los Steps 9a/9b (pagos por operación) se construyeron y luego **se dieron de baja** el 2026-08-27 por el cambio al modelo de suscripción — ver abajo.
...
- **Step 6 (ciclo del pedido, flujo agendado):** wizard `/pedido/nuevo?providerId=` (oficio → detalle + fotos a bucket `job-photos` → ubicación → tipo + medio de pago informativo) → `createJob`. Detalle cliente `/pedido/[id]` y profesional `/pro/pedido/[id]` con `JobStatusTimeline`. Transiciones: `updateJobStatus` (aceptar/iniciar/cancelar) y `completeJob` (precio final informativo + `jobs_completed`). Listas `/pedidos` (cliente) y `/pro/pedidos` (profesional).
...
- **Step 9a (pagos MP, escrow) — REEMPLAZADO (2026-08-27):** dado de baja por el modelo de suscripción (`docs/superpowers/specs/2026-07-31-modelo-suscripcion.md`). Se eliminaron `preference.ts`/`payments.ts`/`refund.ts`/`commission.ts`, la rama de pago de `createJob`, el gate de pago de `updateJobStatus`, la parte de pagos del cron `expire-jobs`, `JobPaymentPanel` y el índice `uq_jobs_mp_payment_id` (migración `0002`). **Se conservan** `lib/mercadopago/webhook.ts` (firma HMAC) y `client.ts`, que se reusan para suscripciones; el endpoint `POST /api/webhooks/mercadopago` sigue validando firma pero no procesa eventos de pago.
...
- **Audit trail + notificaciones + emails (2026-07-16):** (1) **Audit trail**: tabla `audit_logs` (schema Drizzle, migración `0001_bouncy_martin_li.sql`; sin FK a profiles a propósito — el log sobrevive al borrado; `user_id` null en eventos pre-login). `lib/audit.ts`: `logSecurityEvent(action, {userId, metadata})` fire-and-forget (jamás lanza; metadata redactada; emails enmascarados con `maskEmail`), `getRecentSecurityActivity`, `isNewLoginContext` (¿IP+browser nuevos vs últimos 20 logins?; false en primer login) y `getRequestContext` (x-forwarded-for). `lib/security-utils.ts` (puro): `parseUserAgent` + `maskEmail`. Integrado en TODOS los flujos: password login/signup/reset/change, logout/logout_all, OAuth (callback: login, failed_login, identity_link vía nuevo param `flow=link` de `GoogleAccountLink`), unlink (`recordUnlinkAudit` en `lib/actions/audit.ts`), completeProfile y updateAvatar-ready. UI: `SecurityActivity` (últimos 5 eventos con navegador/OS/IP) en `/perfil` y `/pro/perfil`. RLS: solo lectura propia (docs/audit-notifications-setup.sql). (2) **Campanita**: columna `link` agregada a `notifications` (misma migración; los 3 inserts existentes la pueblan). `NotificationsBell` (`components/shared/`) en un header sticky nuevo de los layouts `(app)`/`(pro)`: badge con contador, dropdown animado con skeletons, marcar leída/todas (vía supabase-js + RLS `notif_self_rw`), navegación por `link`, y Realtime INSERT (requiere publicar `notifications` — en el mismo SQL doc). `read_at` hace de `is_read`. (3) **Emails** (`lib/emails/`): `send.ts` (Resend; **no-op sin RESEND_API_KEY**; `EMAIL_FROM` opcional), `layout.ts` (tablas + estilos inline con los design tokens, mobile-first), templates `welcome.ts` (al completar onboarding por primera vez), `security-alert.ts` (login desde IP/dispositivo nuevo, cambio de contraseña, link/unlink de Google — vía `security-events.ts`) y `otp.ts` (los OTP los manda Supabase: pegar `docs/supabase-otp-email-template.html` en Authentication → Email Templates). Tests: `tests/security-emails.test.ts`. **Manual:** `pnpm db:migrate` (o sección 1 del SQL doc) + `docs/audit-notifications-setup.sql` en Supabase; `RESEND_API_KEY`/`EMAIL_FROM` en Vercel para emails reales.
...
- **Step 9b (liquidación al completar) — REEMPLAZADO (2026-08-27):** dado de baja junto con 9a (mismo spec). Se eliminaron la liquidación de `completeJob` (ledger, reintegros parciales, comisión), `commission_ledger`, `provider_mp_tokens`, `/pro/cobros`, `/admin/comisiones` y `settleCommission`. `completeJob` quedó con: precio final (dato de negocio), transición a `completed` e incremento de `jobs_completed`.
...
- **Step 12 (admin):** dashboard `/admin` con KPIs reales (verificados, completados, GMV), `/admin/trabajos` (tabla global con alias de Drizzle para cliente/profesional), `/admin/profesionales` (verificación/rating/online). Verificaciones ya existía (Step 4). (`/admin/comisiones` se dio de baja con 9b.)

Pendiente: implementar el modelo de suscripción (`docs/superpowers/specs/2026-07-31-modelo-suscripcion.md`, sección 11: tabla `subscriptions`, `lib/subscriptions/`, gate en PostGIS y server actions, pantallas y `/admin/suscripciones`) → SEO (Step 13) → polish + deploy (Step 14: perfil cliente editable, `/pro/mensajes`, íconos push).
